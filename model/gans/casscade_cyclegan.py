...
class CasscadeCyclegan(BaseGAN):
    ...
    def __init__(self,
        ...
        self.discriminators['d'] = build_module(discriminator)
        ...
        self.image_buffers['d'] = GANImageBuffer(self.buffer_size)
    ...
    def init_weights(self, pretrained=None):
        ...
        self.discriminators['d'].init_weights(pretrained=pretrained)
    ...
    def forward_train(self, img_a, img_b, img_c, meta):
        ...
        rec_c = generators['d'](fake_c)
        fake_b_ = generators['d'](real_c)
        rec_b_ = generators['c'](fake_b_)
    ...
    def forward_test(self,
        ...
        # fake_a = generators['b'](generators['a'](real_b))
        ...
        if save_image:
            ...
            if self.show_input:
                ...
                output = np.concatenate([
                    ...
                    tensor2img(results['real_c'], min_max=(-1, 1)),
    ...
    def backward_discriminators(self, outputs):
        ...
        losses = dict()
        # GAN loss for discriminators['d']
        fake_b_ = self.image_buffers['d'].query(outputs['fake_b_'])
        fake_pred = discriminators['b'](fake_b_.detach())
        losses['loss_gan_d_c_fake'] = self.gan_loss(fake_pred,
                                                    target_is_real=False,
                                                    is_disc=True)
        real_pred = discriminators['b'](outputs['real_b'])
        losses['loss_gan_d_d_real'] = self.gan_loss(real_pred,
                                                    target_is_real=True,
                                                    is_disc=True)

        loss_d_d, log_vars_d_d = self._parse_losses(losses)
        loss_d_d *= 0.5
        loss_d_d.backward()
        log_vars_d['loss_gan_d_d'] = log_vars_d_d['loss'] * 0.5
    ...
    def backward_generators(self, outputs):
        ...
        if self.id_loss is not None and self.id_loss.loss_weight > 0:
            ...
            id_d = generators['b'](outputs['real_b'])
        ...
        fake_pred = discriminators['b'](outputs['fake_c'])
