...
class RoundGANV3(BaseGAN):
    ...
    def forward_train(self, img_a, img_b, img_c, meta):
        ...
        """
        # necessary setup
        day, twilight, night, _ = self.setup(img_a, img_b, img_c, meta)
        ...
        fake_twilight = generators['a'](day)
        fake_night = generators['b'](twilight)
        fake_day = generators['c'](night)
        rec_day = generators['c'](generators['b'](fake_twilight))
        rec_twilight = generators['a'](generators['c'](fake_night))
        rec_night = generators['b'](generators['a'](fake_day))

        results = dict(real_a=day,
                       fake_b=fake_twilight,
                       rec_a=rec_day,
                       real_b=twilight,
                       fake_a=fake_day,
                       rec_b=rec_twilight,
    ...
    def backward_discriminators(self, outputs):
        ...
        fake_pred = discriminators['b'](fake_c.detach())
        losses['loss_gan_d_b_fake'] = self.gan_loss(fake_pred,
                                                    target_is_real=False,
                                                    is_disc=True)
        real_pred = discriminators['b'](outputs['real_c'])
        ...
        fake_pred = discriminators['c'](fake_a.detach())
        losses['loss_gan_d_c_fake'] = self.gan_loss(fake_pred,
                                                    target_is_real=False,
                                                    is_disc=True)
        real_pred = discriminators['c'](outputs['real_a'])
